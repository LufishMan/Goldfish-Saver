...
一款參考 Windows 便利貼設計的簡易桌面備忘錄程式，使用 Electron 開發，支援 **Markdown 即時編輯**、**待辦提醒（Windows 11 系統通知）**、狀態分類、標籤、顏色標記、釘選、以及像子母畫面（PiP）的浮動「檢視模式」。跨平台支援 **Windows / macOS**。
...
- **待辦提醒**：每則可獨立開啟提醒並設定提醒時間，到點跳出 **Windows 11 系統通知（Toast）**；點通知即喚回視窗並開啟該則。清單上會顯示 ⏰ 提醒徽章（已提醒轉為 🔔）。
- **狀態分類**：待辦 / 處理中 / 已完成三種狀態，側邊分頁**以不同顏色區分**（藍 / 橙 / 綠），切換時有動態效果，標記「已完成」還有 ✓ 慶祝動畫。
- **欄位**：標題、內容、日期、標籤。
- **日期**：支援「單日」或「區間」（區間不可超過 3 個月）；當日的日期會 highlight，過期會以紅色標示。
...
目前採**手動更新**：拿到新版安裝檔後，**直接執行即可就地升級，不必先反安裝**。

### Windows
1. 下載新版 `GoldFish Saver Setup x.x.x.exe`，直接雙擊執行。
2. 安裝程式會自動移除舊版、裝上新版（一鍵完成）。
3. 你的備忘錄與提醒設定**會完整保留**（資料獨立存在 `%APPDATA%\goldfish-saver\`，更新不會清除）。

### macOS
1. 下載新版 `.dmg`，把 App 拖進「應用程式」覆蓋舊的即可。
2. 資料同樣保留在 `~/Library/Application Support/goldfish-saver/`。

> ⚠️ **尚未支援 App 內自動更新**：程式不會自己偵測或提示新版，需由你手動下載新安裝檔更新。若日後要做「開啟時自動檢查並更新」，可加入 `electron-updater` 並將安裝檔發佈到 GitHub Releases。
